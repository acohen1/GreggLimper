...
async def embed_text(text: str, model: str | None = None) -> np.ndarray:
    ...
    Model defaults to Config.EMB_MODEL_ID.
    """
    if not text:
        # Use Config.EMB_DIM if provided; else assume common dims for the model
        dim = Config.EMB_DIM
        return np.zeros(dim, dtype=np.float32)

    use_model = model or Config.EMB_MODEL_ID
    resp = await aoai.embeddings.create(model=use_model, input=text)
    vec = np.asarray(resp.data[0].embedding, dtype=np.float32)
    # Optional safety check if you set EMB_DIM in Config
    dim = Config.EMB_DIM
    if vec.size != dim:
