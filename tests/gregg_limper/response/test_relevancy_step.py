...
async def test_relevancy_step_loop():
    ...
    with patch("gregg_limper.response.steps.relevancy.check_relevancy", new_callable=AsyncMock) as mock_check, \
        ...
        # 1. "Bad Response 1" -> False (Irrelevant)
        # 2. "Bad Response 2" -> False (Irrelevant)
        # 3. "Good Response" -> True (Relevant)
        ...
        # Loop 1: loops=1. Base 0.7 + (1 * 0.2) = 0.9
        # Loop 2: loops=2. Base 0.7 + (2 * 0.2) = 1.1
        
        # Check calls to chat
        # Call 1
        args1, kwargs1 = mock_chat.call_args_list[0]
        assert kwargs1["temperature"] == pytest.approx(0.9)
        
        # Call 2
        args2, kwargs2 = mock_chat.call_args_list[1]
        assert kwargs2["temperature"] == pytest.approx(1.1)
...
async def test_relevancy_step_history_growth():
    ...
    with patch("gregg_limper.response.steps.relevancy.check_relevancy", new_callable=AsyncMock) as mock_check, \
        ...
        # Should be: [User, Assistant(Bad 1), User(Instruction)]
        ...
        assert "irrelevant" in messages_passed[2]["content"]
