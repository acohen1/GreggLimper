...
4. Keep secrets in the environment: set `DISCORD_API_TOKEN`, `OPENAI_API_KEY`, and `PII_SALT` (when `scrub_pii` is enabled) before running the tuner. The TOML intentionally excludes inline secrets.
...
- `dataset.scrub_pii` / `--scrub-pii` (with `dataset.pii_salt` / `--pii-salt`): enable deterministic aliasing to anonymize user identifiers in the exported dataset. When on, every Discord author/mention is replaced with a hashed alias, and `records.metadata.jsonl` records the alias used for the assistant persona. Keep the salt in an environment variable such as `PII_SALT` so it never lands in source control.
